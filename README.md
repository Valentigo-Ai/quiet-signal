...
**Icon** (`assets/brand/icon.png`, `assets/brand/adaptive-icon.png`) - three concentric rings rising from a single dot on a solid terracotta field. The rings are hope reaching outward toward a brighter future; the dot at the center is the person sending it. Rings are tinted in a soft, muted three-tone sweep (dusty rose `#E8A98F`, gold `#E8C468`, sage `#9FCBB8`) rather than plain cream - deliberately not the pride-flag palette (that's 6 fully-saturated colours in horizontal stripes; this is 3 muted tones in concentric rings, no blue or violet) - chosen because a solid-colour icon with a distinctive multi-tone mark stands out further in a store/home-screen grid than the single-flat-colour convention most wellness apps use (Calm's blue circle, Headspace's orange circle), while keeping the bold contrast that research shows drives tap-through. Background `#8A6552`, dot `#F6F1EB`.

**Splash** (`assets/brand/splash.png`) - the same ring-and-dot mark, in the app's normal warm-terracotta-on-cream palette, centered on the app's actual background colour so there's no colour jump between splash and first screen.
